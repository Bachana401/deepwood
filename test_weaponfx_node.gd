...
func _ready() -> void:
	...
	# 103 -> 101: GRIEF MADE SHARP and WORLDSPIKE, the last two T6 plain rungs
	#             that had verbs designed for them.
	var PLAIN_QUOTA := 101
	var plain_n := 0
	var overdressed := []
	var unknown := []
	...
	check("every chained rung forges from kin plus materials (98 links)",
		chained == 98 and broken.is_empty(),

...
func _verb_identity(row: Array) -> String:
	...
	# and a staff's soul is its SLAM. Ten staffs all say "staff" because the
	# extend is their CLASS -- rhythm draws it longer, the fourth blow strikes
	# the earth -- but one punts, one gathers, one drives the herd one way, one
	# raises a stone that stays. Same rule as the whips above, for the same
...
func _ready() -> void:
	...
	for engine_path in ["res://weapon_projectile.gd", "res://weapon_fx.gd",
			...
			# main.gd and the proving ground join the list the day main gained a
			# weapon-testing hotkey: nothing in the kit loads main.gd, so it
	...
	check("every chained rung forges from kin plus materials (20 links)",
		chained == 20 and broken.is_empty(),

...
- **The sky had to be made visible first.** The sun and moon lagged the camera by 88% of however far the player had walked — measured at six screen-widths out at the far end of the village row — so **nobody had ever seen either from their own town, at any hour**; and the ordinary arc peaked above the top of the frame, which put *totality*, the one moment this feature exists for, just past the screen edge. The sky is camera-anchored now (parallax belongs to the ridgelines, which already have it), the arc rides lower, and the eclipse pair is drawn further down still while it runs. Both defects passed static analysis and the full test suite; only a screenshot found them.
- **What it is FOR — The Hollow Sun.** Raising the **Hollow Signet** during a true eclipse calls `evt_hollowsun`: an apex event boss that stands **outside** the ten-boss hunt (§4.2) — so the Hunter's Horn capstone is never gated behind a 3%-a-day sky — and is the only boss in the game fought **in your own streets**, so every trick it owns is one your town is standing inside. Difficulty is mechanics, never one-shots (forever rule) — its damage sits barely past the Master of the Hunt's, its health half again. It pays the **Eclipse set**, the top of the item ladder: Ringbreaker, Corona Edge, the Hollow Crown, Eclipsed Plate, Ringlight. And because calling it endangers the whole village, it is **repeatable by design** — dev, 2026-08-06: *"you're endangering your whole village + progress… it would be very unfair to give 1 chance."*
- **And the danger to the village is now real.** The Hollow Sun is the first boss in Deepwood that can hurt anything but the player: it carries `razes_buildings`, and its **ground** attacks — meteors and pillars — damage whatever hall stands where they land. Until that flag existed the dev's stated stake was decorative, and this boss could stand in the middle of Deepwood all day without scratching a wall. **It wounds and never levels:** each hit is floored at a share of the building's full health and is deliberately not scaled by the boss's damage curve, so a hall can be brought to the brink here and finished only by neglect afterwards. What that costs the town, and how the town repairs it, is §5.16.
- **Obtaining the Signet:** it is **crafted** (`Inventory.CRAFT_RECIPES`), out of deep materials rather than boss trophies — the Horn is the reward for knowing every secret, the Signet is the reward for going far enough down. Nothing drops it, so the recipe is the only road in; before it existed the entire eclipse chain was a dead end. *(This closes the 🟡 open item carried here on 2026-08-06.)*
...
| Posting the watch | ± buttons per block, every session (§5.15) | the Barracks' **Warchief** (`_warchief_posts_the_watch`) | **65** |
...
> **The forty-floor hole this law caught.** Measured against the real rescue depths, **nothing new was automated between floor 56 and floor 95**: the Foreman (60), the Warchief (65/70) and four Lead Researchers (75–90) are all *second holders* of seats already filled — they raise numbers and retire no chore. Meanwhile posting warriors to patrol blocks stayed fully manual to the end of the game. The Warchief takes it, which is exactly what a war leader is for, and it lands in the middle of the dead stretch. **A rung that inflates a stat does not satisfy this law; only one that retires a verb does.**
...
- **The standing rule it must never break:** warriors produce **bulk**, the player produces **meaning**. No blueprints, no rescued people — ever. *(The older form of this rule said "no gear, no relics, nothing else, ever" and did not survive `_patrol_find_gear`, which hands out all three. The absolute is retired in the code, in the patrol panel and here; what stands is the measured claim — a find is a **delight and never a strategy**, roughly one per 6.9 in-game days at twelve posted, and never able to out-reach the floors that earned it.)*
- **The road down is cut for real.** Descending to a floor beyond the shallowest fallen block is refused outright, and the notice names the stretch — *"The stair ends at floors %d–%d. Nothing walks past that stretch — only a Waystone reaches beyond it."* The gate is derived from the swept floors rather than stored, which is why it survives a save; the first version keyed off a block sitting at full creep, and the fall itself zeroes that creep in the very next statement, so the condition could not survive the instant that creates it.
- **THE WARCHIEF TAKES THE WATCH** (§5.1a, rescue depth **65**). With a Warchief seated, `_warchief_posts_the_watch()` garrisons every swept block to the **minimum** strength that out-paces its creep, **shallowest first** — a shallow block is the one whose loss cuts the road, and a deep block held beyond a cut road is worth nothing. It **never overrides a watch you set by hand** and **never empties the wall**, stopping the moment no warrior is spare. It relieves the bookkeeping, not the decision: how much to risk stays yours, which is the difference between automation and being played for.
...
**THE SICKNESS.** Physical and contagious — **it is not corruption** (§10). Despair turns people into demons; this is a disease. *(It is also no longer gated on `CORRUPTION_ENABLED`: flipping that switch to test corruption used to disable the disease along with it.)*
...
**TWO STRAINS 🔒 (dev ruling 2026-08-06) — the same system with early manners and late teeth.**
- **THE ILLNESS CANNOT KILL.** It carries no drain at all. It **stops a body mending** and **drags the town's mood down**, and that is its entire cost. The reasoning is pacing: where this first appears the player has no staffed Hospital, no doctors and no leader to answer it with, so a strain that took people then would be a punishment for being *early* rather than a problem to solve. An annoyance you notice and work around is the correct shape at that stage.
- **THE PLAGUE is the real thing**, gated behind `PLAGUE_MIN_DEPTH` so it only exists once the player is deep enough to fight it properly. It drains, it runs through a packed row harder, it will not be shrugged off unaided, and ignored long enough it takes people. **Only it reaps** — and the reaper guards on the *strain*, never on the HP alone, so a villager sitting at zero from a siege wound is never quietly recorded as a plague death.
- **The strain travels with the carrier**, so an outbreak can never quietly escalate into the late one. The two read differently on purpose (the illness domestic, the plague institutional) and the morale panel counts them apart: a town with three colds and one with three plague cases used to look identical, and the one worth coming home for was the invisible one.

**IMMUNITY — the thing that lets an outbreak END** 🔒 (`IMMUNITY_DAYS`, stamped on **recovery**, never on infection).
- Without it, a single case into a cared-for town of eighty killed **73 of 80** — and a staffed Hospital on the cottage row still lost 55. The cause was not the drain or the spread: a cured villager was re-infected the same night and re-rolled until a roll killed them. **There was no such thing as having had it.** Cure rate and immunity are useless apart (66.7 and 63.5 dead respectively; together, 7.8), and it is immunity that finally makes the Hospital worth building.
- It is the answer to *"what if I cannot beat it?"* — every recovery permanently shrinks the pool the sickness can burn through, so it starves and **always ends**. An unprepared town pays a toll, never the settlement, which keeps this inside the standing rule that a village death is a wound and never a game over.
- 🔒 **The principle outlives the number: IMMUNITY MUST OUTLAST THE OUTBREAK.** A window shorter than an outbreak's own length hands the survivors back as fuel before the thing can starve — the first attempt ended **0 of 10** trials over 1200 in-game days. Not slow: permanently endemic, precisely the failure immunity exists to prevent.

**THE CONTACT GRAPH — homes only** 🔒 (dev ruling 2026-08-06).
- It spreads **house to house**: `_lives_touch` compares the x of the roof each of them sleeps under, within `SICK_SPREAD_RADIUS`, and a child is measured at its parents' door. Cottages packed in a row pass it along fast; a town spread down the road resists it. That gives cottage placement a **second** meaning and makes **where you put the Hospital the most consequential placement in the game** — its ward aura is the only standing defence.
- It used to read `villager_places` — cottage **and** workplace — which stood every colleague at **distance zero**. Measured in a staffed town of eighty: every soul touched all seventy-nine others, and spreading the cottage row twenty-seven times wider changed the graph by nothing at all. That falsified this section's own claim, the claim about the Hospital's placement, and the spatial half of §5.11–5.12 together. Homes-only cuts contacts per soul from 79 to 23.
- **A shared workplace is kept as a separate, weaker vector** (`WORKMATE_INFECT_PER_DAY`) — a real way to catch it, never a zero-distance edge that flattens the map.

- **What the ward does:** inside *The Ward's Shadow* the drain is cut (`SICK_WARD_DRAIN_RELIEF`), the odds of throwing it off jump (`SICK_WARD_CURE_BONUS`), and neighbours in range catch it far less. A staffed Hospital helps even at range, at a fraction of the strength. A death here carries the ordinary death-shock into morale (§10) — once, not twice: the remover already registers it.
...
- **Exempt:** the Ten, the pledged shadows, and anyone still immune (a legend sickens to the brink, never past). Warriors are **not** exempt — unlike corruption, the plague does not care that you can fight.
...
- Both emergencies are shouted at the top of the morale panel — how many are burning and whether anyone is fighting it, how many are ill (and how many of those are **plague**) and whether a ward is holding it — because a disaster you cannot see is just a silent tax on a town you thought was running itself.

**CONDITION AND MENDING — the wound that finally counts ✅ built (2026-08-06).** Both emergencies above, and the Hollow Sun (§4.5), leave halls **standing and hurt**, and until now that state cost nothing and could not be undone.
- **A battered hall works badly.** `is_building_operational()` reads the build *stage* and never the health, so a hall beaten to a fraction produced at full rate, taxed normally and counted for the finale gate. `building_condition()` now scales its output by how whole it is, as the final multiplier on §5.11's placement term. **A wound that neither heals nor hurts is not a wound.**
- **Scaled from a floor, never a cliff** (`CONDITION_FLOOR`): a wreck still contributes, which is what guarantees this can never soft-lock a save or strand the finale gate behind a building nobody can repair.
- **The Builderhouse crew can mend a standing hall** (`_auto_mend_one`), out of the village stores, at the Master Builder's expense-free rate when that power is awake. Deliberately **slow**: a fire or a fight should leave a mark the town heals over days.
- **The hole it closed.** A building only loses a stage at zero health, and health was only restored by *finishing* that last stage — so a hall knocked to a fraction and left standing had no route back to whole by any means in the game. Nothing used to leave a building alive and hurt; sieges either razed or missed. Then fire arrived, and then the Hollow Sun, **whose damage floor exists precisely so it never destroys anything** — which meant its scars could never be repaired either. The mercy was making its own damage permanent.
...
- **Corruption is not sickness** (§5.16). Despair is a morale failure that produces a *demon*; the sickness is physical and contagious, spreads by proximity of **homes** rather than by morale, and in its **late strain only** can kill. They share only the death-shock they both feed. Warriors are exempt from the first and **not** from the second. *(The two are also independent switches now: `CORRUPTION_ENABLED` no longer gates the sickness tick.)*
...
***Revised later the same day (HEAD `d6c7b9e`).*** *Where this book had gone out of date inside a single session: the **Hollow Signet is craftable** and §4.5's 🟡 open item is closed; the Hollow Sun **damages the buildings it fights among** (`razes_buildings`), wounding but never levelling; the sickness is **two strains** — an early illness that cannot kill and a depth-gated plague that can — governed by **immunity** (the standing principle: **immunity must outlast the outbreak**) and by a contact graph that reads **homes only**, a shared workplace being a separate weaker vector; the **Warchief** takes the watch at rescue depth 65, filling §5.1a's forty-floor hole; **building condition and mending** (§5.16) make a wounded hall produce less and give it a road back to whole; and the **sky was made visible from the village at all** (§4.5) — camera-anchored, arc lowered. Every item verified against `game_state.gd`, `boss.gd`, `event_boss.gd`, `inventory.gd`, `day_night_cycle.gd` and `level_select_ui.gd`.*

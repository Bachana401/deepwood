...
1. **Shadow Monarch class kit** — ✅ appears BUILT: the playable class tree is implemented in `skill_tree.gd` (the `nc_*` graph — Legion / Dominion / Ascendant, 3×7 tiers, wired to real effects in `player.gd`: `raise_shade`, `fire_shadow_nova`, `apply_fear_aura`, `_apply_true_form`). Verify it matches your intent; the "needs design" note is stale. (The 7-stage passive of §6 and the Shadow Army finale ability are separate and canon.)

...
const ROWS = [
	...
	["wpn_oakcudgel",    "Oak Cudgel",           "melee", 1, "arc",       8,  0.55, {"rider": "oak"}],
	["wpn_mongrelknife", "Mongrel Knife",        "melee", 1, "arc",       5,  0.28, {"rider": "mongrel"}],
	["wpn_pitshovel",    "Pit Shovel",           "melee", 1, "spadespray", 11, 0.85, {"rung": true}],
	["wpn_barrelstave",  "Barrel Stave",         "melee", 1, "arc",       7,  0.45, {"rider": "stave"}],
	["wpn_rustfang",     "Rustfang",             "melee", 1, "arc",       6,  0.35, {"status": "poison_w", "rider": "rust"}],
	["wpn_fencepike",    "Fence Pike",           "spear", 1, "thrust",    9,  0.8,  {"rider": "fence"}],
	["wpn_eelspear",     "Eelcatcher",           "spear", 1, "thrust",    7,  0.6,  {"rider": "eel"}],
	["wpn_orchardbow",   "Orchard Bow",          "bow",   1, "shot",      7,  0.55, {"rider": "windfall"}],
	["wpn_gutterbow",    "Gutter Bow",           "bow",   1, "shot",      6,  0.45, {"rider": "gutter"}],
	...
	["wpn_lanternblade", "Lanternblade",         "melee", 2, "arc",       10, 0.45, {"status": "burn_w", "rider": "lantern"}],
	["wpn_millsickle",   "Mill Sickle",          "melee", 2, "arc",       9,  0.32, {"rider": "mill"}],
	["wpn_bonepick",     "Bonepick",             "melee", 2, "arc",       12, 0.6,  {"rider": "bone"}],
	["wpn_coldiron",     "Cold Iron Edge",       "melee", 2, "arc",       11, 0.5,  {"status": "slow_w", "rider": "coldiron"}],
	["wpn_ratterdart",   "Ratter's Dart",        "melee", 2, "ratterdart", 10, 0.7,  {"bounces": 2}],
	["wpn_boarspit",     "Boarspit",             "spear", 2, "thrust",    12, 0.75, {"rider": "boar"}],
	["wpn_wallpike",     "Wallwatcher's Pike",   "spear", 2, "thrust",    14, 0.95, {"rider": "wall"}],
	["wpn_reedjavelin",  "Reed Javelins",        "spear", 2, "reedcast",  9, 0.9,  {"count": 2}],
	["wpn_ashbow",       "Ashwood Bow",          "bow",   2, "shot",      10, 0.5,  {"rider": "ash"}],
	...
	["wpn_gatecleaver",  "Gatecleaver",          "spear", 3, "thrust",    17, 0.85, {"rider": "gate"}],
	["wpn_heronlance",   "Heron Lance",          "spear", 3, "thrust",    14, 0.6,  {"rider": "heron"}],
	["wpn_stormprong",   "Stormprong",           "spear", 3, "stormprong", 12, 0.95, {"count": 3}],
	["wpn_veilbow",      "Veilpiercer",          "bow",   3, "shot",      16, 0.6,  {"pierce": true, "rider": "veil"}],
	["wpn_larkbow",      "Lark's Reply",         "bow",   3, "rapid",     7,  0.2,  {"rider": "reply"}],
	...
	["wpn_vespersting",  "Vesper Sting",         "melee", 4, "arc",       13, 0.26, {"status": "poison_w", "rider": "vesper"}],
	...
	["wpn_midnightlance","Midnight Lance",       "spear", 4, "thrust",    19, 0.6,  {"status": "slow_w", "rider": "midnight"}],
	["wpn_curfewbow",    "Curfew Bow",           "bow",   4, "shot",      21, 0.6,  {"pierce": true, "rider": "curfew"}],
	...
	# ---------------- WAVE 3 - TIER 2 (14) ----------------
	["wpn_tannerknife",  "Tanner's Long Knife",  "melee", 2, "arc",       9,  0.3,  {"rider": "tanner"}],
	...
	["wpn_ditchpike",    "Ditchwarden",          "spear", 2, "thrust",    13, 0.85, {"rider": "ditch"}],
	["wpn_frostprong",   "Frostbitten Prong",    "spear", 2, "thrust",    11, 0.7,  {"status": "slow_w", "rider": "prong"}],
	...
	# ---------------- WAVE 3 - TIER 3 (24) ----------------
	["wpn_sextonblade",  "Sexton's Edge",        "melee", 3, "arc",       14, 0.45, {"rider": "sexton"}],
	...
	["wpn_curseknife",   "Cursewright's Knife",  "melee", 3, "arc",       10, 0.28, {"status": "poison_w", "rider": "curse"}],
	...
	["wpn_harrowpike",   "Harrower",             "spear", 3, "thrust",    16, 0.8,  {"rider": "harrow"}],
	["wpn_lamplighter",  "Lamplighter's Reach",  "spear", 3, "thrust",    13, 0.6,  {"status": "burn_w", "rider": "lamp"}],
	...
	["wpn_shrikebow",    "Shrikebow",            "bow",   3, "shot",      15, 0.55, {"rider": "shrike"}],
	...
	["wpn_lightstep",    "Lightstep",            "bow",   3, "rapid",     6,  0.19, {"rider": "lightstep"}],
	...
	# ---------------- WAVE 3 - TIER 4 (28) ----------------
	["wpn_eveningblade", "Evening's Empire",     "melee", 4, "arc",       18, 0.42, {"rider": "empire"}],
	...
	["wpn_palefang",     "Palefang",             "melee", 4, "arc",       12, 0.25, {"status": "slow_w", "rider": "pale"}],
	...
	["wpn_vigilpike",    "Vigil Unbroken",       "spear", 4, "thrust",    22, 0.82, {"rider": "vigil"}],
	["wpn_winterreach",  "Winter's Reach",       "spear", 4, "thrust",    18, 0.6,  {"status": "slow_w", "rider": "winter"}],
	...
	["wpn_gravebow",     "The Polite Reminder",  "bow",   4, "shot",      20, 0.58, {"pierce": true, "rider": "reminder"}],
	["wpn_larkstorm",    "A Storm of Larks",     "bow",   4, "larkstorm", 15, 0.58, {"count": 3}],
	["wpn_needlerain",   "Needlerain",           "bow",   4, "rapid",     8,  0.17, {"rider": "needle"}],
	...
	["wpn_glasstring",   "Glasstring",           "bow",   4, "shot",      17, 0.5,  {"status": "slow_w", "rider": "glass"}],
...
const RIDER_DESC = {
	...
	# the plain-bow souls (2026-07-31): sixteen bows, sixteen small tricks
	"crow":    "The hit CAWS. Everything standing near the one you struck startles back a step -- a bow that herds before it hurts.",
	"gutter":  "Its shafts refuse the wall: the first thing in their way is a lip to SKIP off, flat and low, half the road again.",
	"windfall": "The shaft DROPS as it flies, and what falls hits harder. Loose it high and let the orchard do the rest.",
	"briar":   "Thorns catch best in a wound already open: it hits well harder into anything POISONED -- including by its own string.",
	"dart":    "It gathers heat the whole flight -- weak at your feet, half again as hard at full stretch. Distance is the whetstone.",
	"ash":     "Ash remembers fire. The shaft passes CLEAN through anything already burning, paying it on the way to whatever hides behind.",
	"stoop":   "Loosed while FALLING it is a stoop, and the dive is most of the arrow. The ground is where sparrowhawks go hungry.",
	"sting":   "A wasp's rhythm: three shafts are a warning, and every FOURTH is the sting -- venomed, and half again as hard.",
	"reply":   "It does not start the song. For a breath after you are HURT, every loose is answered by a second shaft.",
	"lightstep": "A walking archer never quite stops: its shafts fly harder while you MOVE, and an anchored stance earns nothing.",
	"veil":    "It parts a body like a curtain and comes out SHARPER -- every layer pierced whets the point for the next.",
	"shrike":  "A kill does not stop the shaft. It goes through the dead to the living, harder, the way a shrike works a hedge.",
	"curfew":  "Everything it passes through is sent HOME -- slowed to half for a heartbeat. The street empties behind the shot.",
	"reminder": "The first hit is a notice. Strike the same body again within three seconds and the second hit lands well harder, as previously discussed.",
	"glass":   "The shaft's DEATH is the weapon: on its last body, or at the end of its road, it shatters onward into three slivers.",
	"needle":  "A miss is not nothing. Any shaft that dies in the air falls straight down as a needle, and the sky keeps what you paid it.",
	# the plain-spear souls (2026-07-31): fourteen thrusts, fourteen facts
	"fence":   "A fence HOLDS. Whatever the point takes stands pinned at the rail for a beat, going nowhere.",
	"eel":     "The only thrust that PULLS: the point hooks the catch a step toward you. What slips in never slips back out.",
	"hay":     "Two honest jabs -- and the third is the HAYMAKER, near double. You can count it, and so can they.",
	"boar":    "A spit is BRACED for the charge: anything coming AT you meets half again the point. Cowards walking away are merely stabbed.",
	"wall":    "A watcher does not wander. Plant your feet and the pike pays a third more; a walking wallwatcher is just a man with a stick.",
	"ditch":   "It works best DOWNWARD, the way a warden clears a ditch: anything below your feet takes half again the thrust.",
	"prong":   "TWO points. The fork finds a second body beside the first and pays it half -- one thrust, a pair of wounds.",
	"gate":    "It does not stop at the body: the blow SLAMS them backward into whoever queues behind, and the queue pays too.",
	"heron":   "The heron fishes by STANDING STILL. Hold the point a breath between strikes and the strike lands near double.",
	"harrow":  "A harrow TURNS the soil: the struck body is dragged through and dropped on your other side. The field never argues.",
	"lamp":    "A lit target is an easy target: the reach pays a third more into anything BURNING -- and its own touch lights them.",
	"midnight": "It keeps its own hours. From the twentieth bell to the fifth, the lance is two-fifths harder -- daylight is someone else's problem.",
	"vigil":   "The vigil holds while YOU are whole: at full health the point is near half again. Bleed, and it is only a spear.",
	"winter":  "Cold SPREADS. Strike a slowed body and its winter passes to the nearest neighbour, uninvited.",
	# the plain-arc souls (2026-07-31): seventeen knives, cudgels and sickles
	"mongrel": "A mongrel fights dirtiest CORNERED: below two-fifths of your health the knife is half again as sharp. Comfort dulls it.",
	"oak":     "Heavy wood argues in one syllable: everything it lands on is SENT, hard, and walks away dazed for a step.",
	"stave":   "The stave remembers the barrel. Every FIFTH blow lands as a hollow BOOM that pays everything standing near the strike.",
	"rust":    "Rust never sleeps: each bite corrodes the body's own blows softer -- three bites deep -- and leaves poison in the groove.",
	"poker":   "Made for stirring fires. Strike a BURNING body and its embers are poked onto whoever stands nearest.",
	"mill":    "The grindstone wants TEMPO: keep each blow within a breath of the last and the mill turns faster and faster.",
	"tanner":  "A tanner works ONE hide at a time: blow after blow on the same body cuts deeper. Switching hides wastes the work.",
	"lantern": "It carries its light past the end: a body it kills leaves a small WATCH-FIRE standing where they fell, burning the curious.",
	"bone":    "A pick for what is nearly carrion: anything under a third of its health takes half again. It finishes arguments.",
	"coldiron": "Cold iron is the OLD ward: elites and the crowned pay two-fifths more. Honest beasts are merely cut.",
	"scales":  "Justice EVENS: any body standing healthier than you pays two-fifths more. The watchman has no quarrel with the broken.",
	"adder":   "The FIRST bite is the adder's: a body never bitten before pays half again. After that it is only a knife -- find fresh skin.",
	"sexton":  "The gravedigger is OWED: every body this edge buries pays two health back. The parish keeps its books.",
	"curse":   "The wright writes in venom: end a POISONED body with this knife and the curse leaps to the nearest living heir.",

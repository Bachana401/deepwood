extends Node
# EYES: TIER 4 (weapon overhaul, 2026-07-29).
...
func _ready() -> void:
	...
	for i in range(1200):
		...
		if p != null:
			break
	if p == null:
		say("EYES-T4: no player"); get_tree().quit(1); return
	await _clear_dialog()
	...
	for spec in [
			["wpn_howlpiece",  "p1_howl",     0.34, 1],
			["wpn_winterwheel","p2_roller",   0.55, 1],
			["wpn_reaperrebuke","p3_reaper",  0.85, 1],
			["wpn_prismbreak", "p4_prism",    0.45, 1],
			["wpn_cometflail", "p5_cometchain", 0.70, 1],
			["wpn_falconoath", "p6_oathdash", 0.40, 1]]:
		p.global_position = _home
		if "velocity" in p: p.velocity = Vector2.ZERO

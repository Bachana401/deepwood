extends Node
# EYES: TIER 7 (weapon overhaul, 2026-07-29).
#
# Split out of tool_eyes_crown.gd, which had grown to ~40 shots and stopped
# finishing inside a sane timeout. One walker per tier from here on.
#   MONARCH_TEST="res://tool_eyes_t7.gd" Godot.exe --path .   (no --headless!)
...
func _ready() -> void:
	...
	for i in range(1200):
		...
		if p != null:
			break
	if p == null:
		say("EYES-T7: no player"); get_tree().quit(1); return
	await _clear_dialog()
	...
	# each entry: id, shot name, settle before the shot, extra swings
	for spec in [
			["wpn_afterlight",   "t1_afterlight",  0.30, 1],
			["wpn_finalanvil",   "t2_anvil",       0.52, 1],
			["wpn_worldthorn",   "t3_worldthorn",  0.30, 1],
			["wpn_cometchain",   "t4_cometchain",  0.95, 1],
			["wpn_stormflock",   "t5_stormflock",  0.45, 1],
			["wpn_dawnchorus",   "t6_dawnchorus",  0.26, 1],
			["wpn_worldsedge",   "t7_worldedge",   0.45, 1],
			["wpn_ascendwheel",  "t8_risingwheel", 0.70, 1],
			["wpn_novatongue",   "t9_nova",        0.55, 1],
			["wpn_silencelash",  "t10_hush",       0.75, 1],
			["wpn_skypillar",    "t11_skypillar",  0.30, 1],

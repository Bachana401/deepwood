...
func _finish_death() -> void:
	# a corpse may finish falling through a PAUSED tree (audit fix): killing the
	# last arrival raider starts the reveal dialogue, which pauses the world --
	# and this fade tween paused with it, freezing the body at full opacity for
	# the whole cutscene. The unit is dead (is_dead is set before this runs),
	# so letting only its death animation keep processing changes nothing else.
